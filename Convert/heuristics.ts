...
const THIRD_PARTY_HINTS = [
  "googletagmanager", "google-analytics", "facebook.net", "hotjar", "intercom",
  "segment", "hubspot", "drift", "clarity.ms", "tiktok", "linkedin", "doubleclick",
];
...
function textOf($: CheerioAPI, sel: string): string[] {
  return $(sel).map((_, el) => $(el).text().replace(/\s+/g, " ").trim()).get().filter(Boolean);
}
...
function runSignals(f: PageFacts, $: CheerioAPI): Signal[] {
  ...
  push("h1-present", "Single clear H1", f.h1s.length === 1,
    f.h1s.length === 0 ? "No H1 on the page. Nothing states the offer in the document outline." : `${f.h1s.length} H1 elements compete for the main claim.`,
    "One H1 states the primary claim.", true, f.h1s[0]);

  const headline = f.h1s[0] ?? "";
  const vague = /next generation|revolutioniz|cutting.edge|world.class|innovat|transform your|unlock|empower/i.test(headline);
  push("h1-specific", "Headline names an outcome", Boolean(headline) && !vague && headline.split(/\s+/).length <= 14,
    "Headline uses category language instead of naming what the visitor gets.",
    "Headline names a concrete outcome.", true, headline);

  push("cta-above-fold", "Primary CTA reachable immediately", f.ctas.some((c) => c.aboveFold),
    "No call to action appears early in the document.",
    "A call to action appears early.", true, f.ctas[0]?.text);

  const ctaLabels = new Set(f.ctas.map((c) => c.text.toLowerCase()));
  push("cta-consistent", "One CTA action, repeated", ctaLabels.size > 0 && ctaLabels.size <= 3,
    `${ctaLabels.size} distinct call to action labels split visitor intent.`,
    "Call to action wording stays consistent.", false, [...ctaLabels].slice(0, 6).join(" / "));

  push("form-short", "Form asks for little", f.formFields.length <= 4,
    `${f.formFields.length} form fields. Each additional field costs completions.`,
    `${f.formFields.length} form fields.`, false,
    f.formFields.map((x) => x.name).join(", "));

  push("nav-leaks", "Navigation does not leak attention", f.navLinks <= 8,
    `${f.navLinks} navigation links give visitors somewhere else to go.`,
    "Navigation is restrained.", false);

  push("proof-present", "Proof on the page", f.proofTerms.length > 0,
    "No reviews, counts, named customers or outcome numbers found.",
    `Proof signals found: ${f.proofTerms.slice(0, 3).join("; ")}`, true);

  push("price-visible", "Pricing addressed", f.priceMentions.length > 0,
    "No pricing or cost signal. Visitors who need it must leave to find it.",
    `Pricing signals: ${f.priceMentions.slice(0, 3).join(", ")}`, false);

  const heroImg = f.images[0];
  push("img-dims", "Images reserve their space", f.images.every((i) => i.hasDims || i.lazy),
    "Images without width and height push layout during load, which hurts CLS.",
    "Images declare dimensions.", false);

  push("hero-not-lazy", "Hero image loads eagerly", !heroImg || !heroImg.lazy,
    "The first image is lazy loaded, which usually delays the LCP element.",
    "The first image is not lazy loaded.", true, heroImg?.src);

  const blocking = f.scripts.filter((x) => !x.async && !x.defer);
  push("scripts-nonblocking", "Scripts do not block render", blocking.length === 0,
    `${blocking.length} render blocking scripts.`,
    "All scripts are async or deferred.", false,
    blocking.slice(0, 4).map((x) => x.src).join(", "));

  const third = f.scripts.filter((x) => x.thirdParty);
  push("third-party-weight", "Third party scripts under control", third.length <= 3,
    `${third.length} third party scripts. Each one is main thread time before the visitor can act.`,
    `${third.length} third party scripts.`, false,
    third.map((x) => new URL(x.src, f.url).hostname).join(", "));

  push("viewport", "Mobile viewport set", $('meta[name="viewport"]').length > 0,
    "No viewport meta tag. The page renders at desktop width on phones.",
    "Viewport meta tag present.", true);

  push("title-length", "Title is usable in search", f.title.length >= 15 && f.title.length <= 60,
    `Title is ${f.title.length} characters.`,
    "Title length is reasonable.", false, f.title);

  push("meta-desc", "Meta description present", f.metaDescription.length > 50,
  ...
  push("img-alt", "Images have alt text", alts === 0,
